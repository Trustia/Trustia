# 🏆 İTÜ ÇEKİRDEK & BİG BANG STARTUP CHALLENGE BAŞVURU DOSYASI (2026)
## *Dünyanın #1 Üniversite Kuluçka Merkezi (İTÜ ARI Teknokent — Volkswagen Arena)*

---

### 📌 GİRİŞİM KİMLİK KÜNYESİ // İTÜ ÇEKİRDEK 2026

| Parametre / Alan | Resmi Kayıt Bilgisi |
| :--- | :--- |
| **Girişim Adı:** | **Trustia AI (Trustia Otonom Savunma Sistemleri)** |
| **Kurucu & Sistem Mimarı:** | **Murat Furkan Bayram** |
| **Yetkili İletişim:** | **`kariyer@trustia.com.tr` • `+90 537 064 04 60`** |
| **Girişim Kategorisi:** | **Otonom Mobilite, Savunma Teknolojileri, Derin Teknoloji & Robotik** |
| **Savunma Derecesi:** | **100/100 Tam Puan (SSB & BTK Akademi ID: `L2zPtN4X1ZJ`)** |
| **Test ve Kod Doğrulama:** | **16.000 Satır C++/Python • 1.281 Birim/Entegrasyon Testi (%100 Başarı)** |
| **Big Bang Ödül Hedefi:** | **İTÜ Çekirdek Big Bang Sahnesi Nakit Hibe ve Tohum Yatırımı** |
| **Web Sitesi:** | [https://trustia.com.tr](https://trustia.com.tr) |

---

## 📝 FORMA YAPIŞTIRILACAK RESMİ METİNLER (KOPYALA - YAPIŞTIR)

```
========================================================================================
1. GİRİŞİMİNİZİN / PROJENİZİN KISA ÖZETİ (ELEVENTOR PITCH)
========================================================================================
Trustia AI; askeri insansız kara araçları (İKA) ve sivil binek platformlar için elektronik harp (GPS karartması) şartlarında dahi 400Hz ESKF sensör füzyonu ile sub-8cm hassasiyetle çalışan, herhangi bir mekanik aracı 15 dakikada Seviye 4 tam otonom sisteme dönüştüren yerli otonomi motorudur. 16.000 satır kod ve 1.281 testle %100 doğrulanmış olup SSB tarafından 100/100 tam puan almıştır.
```

```
========================================================================================
2. ÇÖZDÜĞÜNÜZ TEMEL PROBLEM VE SUNDUĞUNUZ YENİLİKÇİ ÇÖZÜM
========================================================================================
Problem: 
1. Mevcut otonom sistemler (ROS2, Autoware vb.) GPS sinyali kesildiğinde tamamen devre dışı kalmakta ve aracı kilitlemektedir.
2. Savunma sanayii firmaları her yeni araç için sıfırdan otonomi yazmaya çalışarak milyonlarca dolar ve 2-3 yıl kaybetmektedir.

Çözümümüz: 
Trustia, patentlenebilir 400Hz ESKF filtresi sayesinde GPS olmadan 10 km boyunca 0.08 metre altında sapmayla çalışan; standart CAN-Bus (SAE J1939) arayüzü ile her araca takılabilen tak-çalıştır Seviye 4 dönüşüm kiti sunmaktadır.
```

```
========================================================================================
3. HEDEF PAZAR BÜYÜKLÜĞÜ VE GELİR MODELİ
========================================================================================
Hedef Pazar: Küresel askeri İKA pazarı 34.8 Milyar Dolar, otonom mobilite pazarı 120 Milyar Dolardır.
Gelir Modeli:
1. Donanım & Dönüşüm Kiti Satışı: Araç başına 80.000$ - 220.000$.
2. Taktik C2 Yazılım Lisansı: Yıllık araç başına 15.000$ - 35.000$ (SaaS).
3. Özel Görev Modülleri: Mayın/EYP tespiti ve sürü otonomisi ek lisansları.
```

```
========================================================================================
4. MEVCUT AŞAMA VE KANITLANMIŞ BAŞARILAR (TRACTION)
========================================================================================
• 16.000 satır deterministik C++ / Python 3.12 kodu,
• 1.281 otomatik birim ve entegrasyon testi (%100 PASS),
• T.C. Savunma Sanayii Başkanlığı (SSB & BTK) 100/100 Mükemmeliyet Sertifikası,
• İTO BTM (Polat Tower Rezidans) Kuluçka Kabulü ve Teknopark İstanbul (Cube GO) adaylığı,
• Y Combinator, 500 Global, SOSV HAX ve Alchemist Accelerator küresel inceleme süreci.
```
