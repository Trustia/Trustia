...
def run_gui():
    print("[1/1] TRUSTIA Taktik Masaustu Konsolu Baslatiliyor...")
    from command.tactical_gui import main as gui_main
    gui_main()


def run_audit():
    print("[1/1] TRUSTIA %100 Yerli Katki AST Sertifikasyon Denetimi Calistiriliyor...")
    from core.certification import main as cert_main
    cert_main()


def run_tests():
    print("[1/1] TRUSTIA 1.273 Otomatik Birim Test Suite Kosturuluyor...")
    subprocess.run([sys.executable, "-m", "pytest"])


def run_threats():
    print("[1/1] TRUSTIA Askeri EYP, Mayin ve KHKN Tehdit Analiz Motoru Calistiriliyor...")
    ...
    print(f"\n[OK] {len(threats)} Adet Askeri Tehdit Dogrulandi ve Karantinaya Alindi:")
    for t in threats:
        print(f"  * [{t.threat_id}] {t.explosive_type.name:<15} | Guven: %{t.confidence*100:.0f} | Karantina: {t.safety_radius_m:.1f}m")


def main():
    parser = argparse.ArgumentParser(description="TRUSTIA Milli Otonomi Platformu Uretim CLI")
    parser.add_argument("command", choices=["gui", "audit", "test", "threats"], help="Calistirilacak komut")
