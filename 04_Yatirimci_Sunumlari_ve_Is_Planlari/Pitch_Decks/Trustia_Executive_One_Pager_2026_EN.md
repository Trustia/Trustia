# TRUSTIA AI
## EXECUTIVE ONE-PAGER / COMPANY TEASER (2026)

**Company:** Trustia AI (Dual-Use Autonomy Platform)  
**Website:** [https://trustia.com.tr](https://trustia.com.tr) | **HQ:** İTO BTM Fulya Campus, Şişli / Istanbul, TR  
**Founding Team:** Murat Furkan Bayram (Founder & CEO), Doğukan Bayram (Co-Founder & Operations)  
**Email:** `iletisim@trustia.com.tr` | `kariyer@trustia.com.tr`  
**Industry:** Defense Technology, Level-4 Robotaxi Mobility & Autonomous Navigation

---

### 🛡️ EXECUTIVE SUMMARY
**Trustia AI** develops the core autonomy operating software for Unmanned Ground Vehicles (UGVs) and passenger electric vehicles operating in **GPS-denied, electronic-warfare-heavy, and dense urban environments**. 

While vehicle manufacturers build robust physical hardware, they lack the proprietary software stack required for true commercial autonomy. Trustia bridges this gap by providing a **hardware-agnostic, zero-external-dependency software brain** that integrates centimeter-level 3D SLAM, kinematic trajectory planning, multi-modal IED/mine AI detection, and drive-by-wire actuation into any vehicle via standard CAN-FD and ROS 2 protocols.

---

### ⚔️ THE CORE TECHNOLOGY MATRIX

```
+-----------------------------------------------------------------------------------+
|                           TRUSTIA AUTONOMY STACK (CORE)                           |
+-----------------------------------------------------------------------------------+
|  1. GPS-Denied 3D SLAM      | Centimeter-accurate LiDAR & Visual Odometry         |
|  2. Kinematic Hybrid A*     | Sub-15ms dynamic route generation with slip angles  |
|  3. AI Threat Defense       | GPR & Thermal detection for IEDs, Mines, CBRN gases |
|  4. Tactical Swarm C2       | Multi-UGV Wedge, Line, Column & Diamond formations  |
|  5. Cyber & Fail-Safe       | HMAC-SHA256 encrypted telemetry, 3s LinkLoss RTH    |
+-----------------------------------------------------------------------------------+
|  STANDARDS: NATO STANAG 4586 Level 4 | SAE AS6091 (JAUS) | ISO 26262 ASIL-D Safe |
+-----------------------------------------------------------------------------------+
```

---

### 🚀 PROVEN TRACTION & TECHNICAL VALIDATION
* **1,301 Automated Tests Passing (100%):** Complete coverage across Unit, Integration, Determinism, and Scenario Matrix pipelines.
* **50/50 Extreme Mission Simulation Campaign:** Zero collisions and 100% mission completion rate across patrol, GPS-denied, obstacle, and combat logistics scenarios.
* **Battle-Ready C2 Console:** MIL-STD-2525 tactical desktop command GUI deployed for live fleet control.
* **Institutional Governance:** Clean founder equity (Murat 80%, Doğukan 20%) and KOSGEB / Defense Academy certifications.
* **85+ Global Applications:** Over 85 institutional applications and registrations submitted across 15+ countries (USA, UK, EU, Qatar, UAE, Saudi Arabia, Germany, South Korea, Turkey).
* **European Union Project Registration:** Official EU Participant Identification Code (PIC: `861711529`) via European Commission. Partner ID `CUS15554` at EIT Urban Mobility. €100,000 grant application submitted (ID: `3.1.02-1206-3732.3`).
* **Qatar Foundation QSTP:** Two applications submitted — $30M Tech Venture Fund and 4-week Doha Sprint Incubation Program.
* **Z Fellows (Silicon Valley):** Selected for live 10-minute Zoom interview with Grace Kasten (Partner at Pace Capital, $10k equity-free grant).
* **Defense Supply Chain Pipeline:** Official supplier evaluations active at ASELSAN (0050569CCE941FD1A49FCEFB9B7BE7D6) and BAYKAR Teknoloji.
* **Crunchbase Verified:** Heat Score 85, CB Rank 534k, Active $500k Pre-Seed round (crunchbase.com/organization/trustia-ai).

---

### 💼 BUSINESS MODEL & DUAL-LAYER PRICING
* **Tier 1: Civil Level-4 Kit:** $18,500 Kit (BOM: $11,500) | $250/mo AaaS subscription.
* **Tier 2: Robotaxi Turnkey Kit:** $35,000 Kit (BOM: $22,800) | $450/mo AaaS or $0.18/km (14-Month Payback).
* **Tier 3: NATO Defense UGV Kit:** $55,000 Kit (BOM: $32,800) | $1,200/mo AaaS | $250,000 – $1,000,000 Bespoke PoC.

---

### 🎯 TARGET MARKET & COMPETITIVE MOAT
* **$2.3T TAM:** Combined Global Autonomous Mobility & Defense Robotics Market by 2030.
* **Proprietary Moat:** 100% zero third-party library dependencies (AST-audited), eliminating foreign backdoors and licensing liabilities.

---

### 💰 THE INVESTMENT OPPORTUNITY
* **Target Round:** **$500,000 Pre-Seed** via SAFE note (Valuation Cap: **$5.0 Million Post-Money**, 20% Discount).
* **Capital Allocation:** 45% ($225k) Hardware & Sensor Test Fleet | 35% ($175k) Core Engineering (18 Months) | 15% ($75k) Proving Grounds & Field Trials | 5% ($25k) IP & ASIL-D Validation.

---
*Confidential & Proprietary © 2026 Trustia AI. All rights reserved.*
