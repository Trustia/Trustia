...
export default function PolicyDocumentView({
  ...
  // Filter unique slugs for sidebar (avoid duplicate gizlilik/kvkk)
          ...
          <aside className="lg:col-span-4 space-y-4">
            <div className="sticky top-28 p-5 rounded-2xl bg-[#0c0f16] border border-white/10 space-y-4 shadow-xl">
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                <span className="font-mono text-xs font-bold text-[#C8FF00] uppercase tracking-wider">
              ...
              <div className="p-3.5 rounded-xl bg-white/[0.02] border border-white/10 text-[11px] text-slate-400 leading-relaxed font-mono">
                <div className="text-white font-bold mb-1 flex items-center gap-1.5">
                  ...
                  <span>{lang === "tr" ? "HUKUKİ YETERLİLİK" : "LEGAL COMPLIANCE"}</span>
                </div>
                {lang === "tr"
                  ? "Bu doküman ulusal mevzuatlar ve uluslararası savunma sanayii standartlarına tam uyumlu resmi politikadır."
                  : "This document constitutes an official corporate policy compliant with international defense standards."}
              </div>
            </div>
          </aside>
          ...
          <div className="lg:col-span-8 space-y-8">
              ...
              <div className="relative z-10 w-full">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <h4 className="font-mono text-xs font-bold text-[#C8FF00] uppercase tracking-wider">
                    {lang === "tr" ? "KURUMSAL ÖZET BİLDİRİMİ" : "EXECUTIVE SUMMARY"}
                  </h4>
                  <button
                  ...
                  >
                    ...
                    <span>{lang === "tr" ? "YAZDIR / İNDİR" : "PRINT / PDF"}</span>
                  </button>
                </div>
            ...
            {activeData.sections.map((section: any, idx: number) => (
              <div key={idx} className="space-y-4 p-8 rounded-2xl bg-[#0c0e14] border border-white/10 hover:border-white/20 transition-colors">
                <h3 className="text-lg sm:text-xl font-bold text-white tracking-tight flex items-center gap-2.5 border-b border-white/10 pb-4">
                  <CheckCircle2 className="w-5 h-5 text-[#C8FF00] shrink-0" />
                  <span>{section.heading}</span>
                </h3>
                <ul className="space-y-3 pt-2">
                  {section.items.map((item: string, itemIdx: number) => (
                    <li key={itemIdx} className="flex items-start gap-3 text-slate-300 text-xs sm:text-sm leading-relaxed">
